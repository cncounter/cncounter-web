...
可能看到我们使用的是大写字母来表示的变量. 这是我个人的习惯, 约定在整个JavaScript程序的生命周期中, 这个变量的值都不会发生改变, 调试程序的时候又希望能手工改变图形的大小。 变量的命名并不影响程序的结果。 我们也将本垒的坐标(`x`和`y`坐标）放到一个对象中, 而不是分散写成 `HOME_PLATE_X`和`HOME_PLATE_Y`这种形式。
